...
window.mineralPowderCalculations.calculateHumidity = function() {
    ...
    if (plateBefore > 0 && plateAfter > 0 && plateWeight > 0 && plateAfter > plateWeight) {
        // Влажность = (до - после) / (после - чаша) * 100
        const humidity = ((plateBefore - plateAfter) / (plateAfter - plateWeight)) * 100;
        
        const field = document.getElementById('humidity_value');
        if (field) {
            field.value = humidity.toFixed(2);
            
            // Валидация (обычно влажность должна быть < 1%)
            if (humidity > 1) {
