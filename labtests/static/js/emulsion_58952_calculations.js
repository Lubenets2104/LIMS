...
window.emulsionCalculations.calculateBinderContent = function() {
    ...
    if (w1 >= 0 && w2 > 0 && w3 >= 0) {
        ...
        if (field) {
            ...
            window.emulsionCalculations.calculateMineralMaterialAdhesion();
        }
    }
};

// === АДГЕЗИЯ К МИНЕРАЛЬНОМУ МАТЕРИАЛУ ===
window.emulsionCalculations.calculateMineralMaterialAdhesion = function() {
    // Берем значение из поля "Содержание вяжущего с эмульгатором"
    const binderContent = parseFloat(document.getElementById('binder_content_value')?.value) || 0;
    
    console.log('calculateMineralMaterialAdhesion, содержание вяжущего:', binderContent);
    
    if (binderContent > 0) {
        // Формула: Масса эмульсии = 10 / binderContent * 100
        const result = (10 / binderContent * 100).toFixed(1);
        
        const field = document.getElementById('mineral_material_adhesion');
        if (field) {
            field.value = result;
            console.log(`Масса эмульсии для минерального материала = ${result} г`);
        }
    } else {
        // Если содержание вяжущего не рассчитано или = 0
        const field = document.getElementById('mineral_material_adhesion');
        if (field) {
            field.value = '';
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        ...
        window.emulsionCalculations.calculateMineralMaterialAdhesion(); // Добавляем расчет адгезии
